...
import {
  ...
  ChevronRight, BarChart3, Users, Truck,
} from "lucide-react";
...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-6 px-5 py-3 rounded-full transition-all duration-500 ${scrolled ? "bg-[#0D0D12]/85 backdrop-blur-xl border border-[#C9A84C]/20 shadow-[0_8px_32px_rgba(0,0,0,0.5)]" : "bg-transparent border border-transparent"}`}>
      <span className="font-inter font-black text-[#FAF8F5] tracking-tight text-sm">NEXUS</span>
      ...
      <button className="btn-magnetic relative overflow-hidden bg-[#C9A84C] text-[#0D0D12] text-xs font-inter font-semibold px-4 py-2 rounded-full">
    ...
    <section ref={heroRef} className="relative h-[100dvh] flex flex-col justify-end pb-20 px-6 md:px-16 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl">
        ...
        <div ref={els[4]} className="flex flex-wrap items-center gap-4 opacity-0">
          <button className="group relative overflow-hidden inline-flex items-center gap-2 bg-[#C9A84C] text-[#0D0D12] font-inter font-semibold px-6 py-3.5 rounded-full shadow-[0_0_40px_rgba(201,168,76,0.3)] hover:scale-[1.03] transition-transform duration-300">
...
function Pricing() {
  const ref = useRef(null);
  const plans = [
    { name: "Essentiel", price: "Gratuit", period: "7 jours", desc: "Testez Nexus sans engagement ni carte bancaire.", features: ["Facturation mobile", "50 clients max", "Gestion de stock", "1 utilisateur"], cta: "Commencer gratuitement", featured: false },
        ...
        <div className="grid md:grid-cols-3 gap-4 items-stretch">
          {plans.map((p) => (
            <div key={p.name} className={`price-card opacity-0 rounded-[2rem] p-7 flex flex-col gap-5 ${p.featured ? "border-2 border-[#C9A84C]/40 shadow-[0_0_60px_rgba(201,168,76,0.12)] md:scale-[1.02]" : "border border-white/7"}`}
              ...
              <button className="relative overflow-hidden group w-full py-3.5 rounded-xl font-inter font-semibold text-sm transition-all hover:scale-[1.02]"
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid md:grid-cols-4 gap-10 mb-12">
          <div className="md:col-span-2">
            <span className="font-inter font-black text-[#FAF8F5] text-2xl tracking-tight">NEXUS</span>
            <p className="font-inter text-[#FAF8F5]/30 text-xs mt-3 leading-relaxed max-w-xs">La plateforme de gestion commerciale conçue pour les entrepreneurs africains qui veulent croître sans se perdre.</p>
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="font-mono text-[10px] text-[#FAF8F5]/18">© 2025 NEXUS · Tous droits réservés</p>
